...
Сначала сделаем схему планируемой сети с указанием типо и номеров портов подключенных устройств, соответствующие физическому уровню модели OSL(L1). Делаем всё в Dia.

![Физические устройства сети с номерами портов(L1)](image/10.128.0.0_L1.jpg){#fig:001 width=70%}

В качестве оборудования уровня ядра используем маршрутизатор Циско 2811,на уровне распределения — коммутаторы Cisco 2960 с возможностью настройки VLAN, а на уровне доступа — коммутаторы Cisco 2950.
...
Теперь построим схему сети и с указанием номеров VLAN, соответствующая канальному уровню модели OSI (L2)

![Схема VLAN сети(L2)](image/10.128.0.0_L2.jpg){#fig:002 width=70%}

Далее определим адресное пространство, ассоциированное с выделенными VLAN. Сделаем схему сети, соответствующую сетевому уровню модели OSI (L3)

![Схема маршрутизации сети (Layer 3)](image/10.128.0.0_L3.jpg){#fig:003 width=70%}
...
| msk-donskaya-yazgeldi-sw-2    | g0/1        | msk-donskaya-sw-1    |             | 2, 3                     |
|                                  | g0/2        | msk-donskaya-sw-3    |             | 2, 3                     |
|                                  | f0/1        | Web-server           | 3           |                          |
|                                  | f0/2        | File-server          | 3           |                          |
| msk-donskaya-yazgeldi-sw-3    | g0/1        | msk-donskaya-sw-2    |             | 2, 3                     |
|                                  | f0/1        | Mail-server          | 3           |                          |
|                                  | f0/2        | Dns-server           | 3           |                          |
| msk-donskaya-yazgeldi-sw-4    | g0/1        | msk-donskaya-sw-1    |             | 2, 101, 102, 103, 104    |
|                                  | f0/1–f0/5   | dk                   | 101         |                          |
|                                  | f0/6–f0/10  | departments          | 102         |                          |
|                                  | f0/11–f0/15 | adm                  | 103         |                          |
|                                  | f0/16–f0/24 | other                | 104         |                          |
| msk-pavlovskaya-yazgeldi-sw-1 | f0/24       | msk-donskaya-sw-1    |             | 2, 101, 104              |
|                                  | f0/1–f0/15  | dk                   | 101         |                          |
|                                  | f0/20       | other                | 104         |                          | 
...
Выполним аналогичную планировку сети для двух других сетей: 172.16.0.0/12 и 192.168.0.0/16 (сеть класса С). Физический и канальный уровни остаются теми же, меняется только сетевой уровень. Рисуем схемы маршрутизации для этих сетей.

![Схема маршрутизации сети 172.16.0.0/12(Layer 3)](image/172.16.0.0_L3.jpg){#fig:004 width=70%}

![Схема маршрутизации сети 192.168.0.0/16(Layer 3)](image/192.168.0.0_L3.jpg){#fig:005 width=70%}
